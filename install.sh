...
for jar in $(find $HADOOP_LIB_DIR -name "*.jar"); do
  if [ "`echo $jar | grep slf4j`" != "" ] || [ "`echo $jar | grep netty`" != "" ] || [ "`echo $jar | grep guava`" != "" ]; then
    continue
  fi
  echo "linking $jar..."
...
if [[ $VERSION == 5.* ]]; then
  ...
  async_link=http://repository.mapr.com/nexus/content/groups/mapr-public/org/hbase/asynchbase/1.7.0-mapr-1603/asynchbase-1.7.0-mapr-1603.jar
